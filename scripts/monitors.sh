...
if [[ "$HOSTNAME" == "nixos-galaxy" ]]; then
    ...
    exec /run/current-system/sw/bin/swaymsg workspace 1
    exec /run/current-system/sw/bin/swaymsg workspace 4

    exec /run/current-system/sw/bin/swaymsg workspace 3
    exec /run/current-system/sw/bin/swaymsg workspace 1
