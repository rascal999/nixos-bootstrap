...
if [[ "$HOSTNAME" == "nixos-galaxy" ]]; then
    exec /run/current-system/sw/bin/swaymsg workspace 1 output HDMI-A-1
    exec /run/current-system/sw/bin/swaymsg workspace 2 output DP-1
    exec /run/current-system/sw/bin/swaymsg workspace 3 output eDP-1
